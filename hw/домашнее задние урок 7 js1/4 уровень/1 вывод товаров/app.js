...
function clickHandler(event) {
    ...
    div.innerHTML = ''
    let product = event.currentTarget.dataset.type
...
function showCategory(category) {
    getProductMarkup(products[category][0])
}    
...
function getProductMarkup(product) {
    ...
    div.insertAdjacentHTML('afterbegin', `<div>${product.name}</div><img src="${product.imageUrl}" alt=""><div>${product.price}</div><a href="https://example.com/producs/${product.id}">Подробнее</a>`)
}
